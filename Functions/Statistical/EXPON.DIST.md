...
## EXPON.DIST Description

EXPON.DIST performs statistical analysis and calculations for data summarization and insights.

> [!f(x)] EXPON.DIST Syntax
>
> ```spreadsheets
> EXPON.DIST(input_value, [options])
> ```
>
> **Parameters:**
...
> [!f(x)] EXPON.DIST Examples
>
> ```spreadsheets
> EXPON.DIST(A1) → result // Basic calculation
> 
> EXPON.DIST(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with EXPON.DIST for conditional logic and decision making:*
```spreadsheets
=IF(EXPON.DIST(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies EXPON.DIST to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with EXPON.DIST for aggregate calculations across multiple results:*
```spreadsheets
=SUM(EXPON.DIST(A1:A5),EXPON.DIST(B1:B5),EXPON.DIST(C1:C5))
```
This formula calculates EXPON.DIST for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with EXPON.DIST for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(EXPON.DIST(A1:A10))
```
This formula combines AVERAGE and EXPON.DIST for comprehensive data analysis
