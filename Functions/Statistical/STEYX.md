...
STEYX performs statistical analysis and calculations for data summarization and insights.

> [!f(x)] STEYX Syntax
>
> ```spreadsheets
> STEYX(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> STEYX(A1) → result // Basic calculation
> 
> STEYX(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with STEYX for conditional logic and decision making:*
```spreadsheets
=IF(STEYX(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies STEYX to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with STEYX for aggregate calculations across multiple results:*
```spreadsheets
=SUM(STEYX(A1:A5),STEYX(B1:B5),STEYX(C1:C5))
```
This formula calculates STEYX for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with STEYX for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(STEYX(A1:A10))
```
This formula combines AVERAGE and STEYX for comprehensive data analysis
