...
F.INV performs statistical analysis and calculations for data summarization and insights.

> [!f(x)] F.INV Syntax
>
> ```spreadsheets
> F.INV(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> F.INV(A1) → result // Basic calculation
> 
> F.INV(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with F.INV for conditional logic and decision making:*
```spreadsheets
=IF(F.INV(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies F.INV to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with F.INV for aggregate calculations across multiple results:*
```spreadsheets
=SUM(F.INV(A1:A5),F.INV(B1:B5),F.INV(C1:C5))
```
This formula calculates F.INV for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with F.INV for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(F.INV(A1:A10))
```
This formula combines AVERAGE and F.INV for comprehensive data analysis
