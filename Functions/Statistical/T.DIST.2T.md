...
## T.DIST.2T Description

T.DIST.2T performs statistical analysis and calculations for data summarization and insights.

> [!f(x)] T.DIST.2T Syntax
>
> ```spreadsheets
> T.DIST.2T(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> T.DIST.2T(A1) → result // Basic calculation
> 
> T.DIST.2T(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with T.DIST.2T for conditional logic and decision making:*
```spreadsheets
=IF(T.DIST.2T(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies T.DIST.2T to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with T.DIST.2T for aggregate calculations across multiple results:*
```spreadsheets
=SUM(T.DIST.2T(A1:A5),T.DIST.2T(B1:B5),T.DIST.2T(C1:C5))
```
This formula calculates T.DIST.2T for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with T.DIST.2T for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(T.DIST.2T(A1:A10))
```
This formula combines AVERAGE and T.DIST.2T for comprehensive data analysis
