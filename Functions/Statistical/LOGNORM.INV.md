...
LOGNORM.INV performs statistical analysis and calculations for data summarization and insights.

> [!f(x)] LOGNORM.INV Syntax
>
> ```spreadsheets
> LOGNORM.INV(input_value, [options])
> ```
>
> **Parameters:**
...
> [!f(x)] LOGNORM.INV Examples
>
> ```spreadsheets
> LOGNORM.INV(A1) → result // Basic calculation
> 
> LOGNORM.INV(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with LOGNORM.INV for conditional logic and decision making:*
```spreadsheets
=IF(LOGNORM.INV(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies LOGNORM.INV to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with LOGNORM.INV for aggregate calculations across multiple results:*
```spreadsheets
=SUM(LOGNORM.INV(A1:A5),LOGNORM.INV(B1:B5),LOGNORM.INV(C1:C5))
```
This formula calculates LOGNORM.INV for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with LOGNORM.INV for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(LOGNORM.INV(A1:A10))
```
This formula combines AVERAGE and LOGNORM.INV for comprehensive data analysis
