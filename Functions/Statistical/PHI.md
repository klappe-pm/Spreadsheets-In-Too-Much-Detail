...
PHI performs statistical analysis and calculations for data summarization and insights.

> [!f(x)] PHI Syntax
>
> ```spreadsheets
> PHI(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> PHI(A1) → result // Basic calculation
> 
> PHI(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with PHI for conditional logic and decision making:*
```spreadsheets
=IF(PHI(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies PHI to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with PHI for aggregate calculations across multiple results:*
```spreadsheets
=SUM(PHI(A1:A5),PHI(B1:B5),PHI(C1:C5))
```
This formula calculates PHI for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with PHI for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(PHI(A1:A10))
```
This formula combines AVERAGE and PHI for comprehensive data analysis
