...
T.DIST performs statistical analysis and calculations for data summarization and insights.

> [!f(x)] T.DIST Syntax
>
> ```spreadsheets
> T.DIST(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> T.DIST(A1) → result // Basic calculation
> 
> T.DIST(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with T.DIST for conditional logic and decision making:*
```spreadsheets
=IF(T.DIST(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies T.DIST to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with T.DIST for aggregate calculations across multiple results:*
```spreadsheets
=SUM(T.DIST(A1:A5),T.DIST(B1:B5),T.DIST(C1:C5))
```
This formula calculates T.DIST for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with T.DIST for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(T.DIST(A1:A10))
```
This formula combines AVERAGE and T.DIST for comprehensive data analysis
