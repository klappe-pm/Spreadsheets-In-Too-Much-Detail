...
QUARTILE performs statistical analysis and calculations for data summarization and insights.

> [!f(x)] QUARTILE Syntax
>
> ```spreadsheets
> QUARTILE(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> QUARTILE(A1) → result // Basic calculation
> 
> QUARTILE(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with QUARTILE for conditional logic and decision making:*
```spreadsheets
=IF(QUARTILE(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies QUARTILE to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with QUARTILE for aggregate calculations across multiple results:*
```spreadsheets
=SUM(QUARTILE(A1:A5),QUARTILE(B1:B5),QUARTILE(C1:C5))
```
This formula calculates QUARTILE for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with QUARTILE for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(QUARTILE(A1:A10))
```
This formula combines AVERAGE and QUARTILE for comprehensive data analysis
