...
BETA.DIST performs statistical analysis and calculations for data summarization and insights.

> [!f(x)] BETA.DIST Syntax
>
> ```spreadsheets
> BETA.DIST(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> BETA.DIST(A1) → result // Basic calculation
> 
> BETA.DIST(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with BETA.DIST for conditional logic and decision making:*
```spreadsheets
=IF(BETA.DIST(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies BETA.DIST to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with BETA.DIST for aggregate calculations across multiple results:*
```spreadsheets
=SUM(BETA.DIST(A1:A5),BETA.DIST(B1:B5),BETA.DIST(C1:C5))
```
This formula calculates BETA.DIST for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with BETA.DIST for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(BETA.DIST(A1:A10))
```
This formula combines AVERAGE and BETA.DIST for comprehensive data analysis
