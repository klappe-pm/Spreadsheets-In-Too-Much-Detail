...
QUARTILE.INC performs statistical analysis and calculations for data summarization and insights.

> [!f(x)] QUARTILE.INC Syntax
>
> ```spreadsheets
> QUARTILE.INC(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> QUARTILE.INC(A1) → result // Basic calculation
> 
> QUARTILE.INC(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with QUARTILE.INC for conditional logic and decision making:*
```spreadsheets
=IF(QUARTILE.INC(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies QUARTILE.INC to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with QUARTILE.INC for aggregate calculations across multiple results:*
```spreadsheets
=SUM(QUARTILE.INC(A1:A5),QUARTILE.INC(B1:B5),QUARTILE.INC(C1:C5))
```
This formula calculates QUARTILE.INC for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with QUARTILE.INC for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(QUARTILE.INC(A1:A10))
```
This formula combines AVERAGE and QUARTILE.INC for comprehensive data analysis
