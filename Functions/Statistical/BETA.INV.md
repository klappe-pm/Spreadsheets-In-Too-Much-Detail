...
## BETA.INV Description

BETA.INV performs statistical analysis and calculations for data summarization and insights.

> [!f(x)] BETA.INV Syntax
>
> ```spreadsheets
> BETA.INV(input_value, [options])
> ```
>
> **Parameters:**
...
> [!f(x)] BETA.INV Examples
>
> ```spreadsheets
> BETA.INV(A1) → result // Basic calculation
> 
> BETA.INV(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with BETA.INV for conditional logic and decision making:*
```spreadsheets
=IF(BETA.INV(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies BETA.INV to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with BETA.INV for aggregate calculations across multiple results:*
```spreadsheets
=SUM(BETA.INV(A1:A5),BETA.INV(B1:B5),BETA.INV(C1:C5))
```
This formula calculates BETA.INV for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with BETA.INV for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(BETA.INV(A1:A10))
```
This formula combines AVERAGE and BETA.INV for comprehensive data analysis
