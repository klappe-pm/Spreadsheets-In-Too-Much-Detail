...
## F.DIST.RT Description

F.DIST.RT performs statistical analysis and calculations for data summarization and insights.

> [!f(x)] F.DIST.RT Syntax
>
> ```spreadsheets
> F.DIST.RT(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> F.DIST.RT(A1) → result // Basic calculation
> 
> F.DIST.RT(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with F.DIST.RT for conditional logic and decision making:*
```spreadsheets
=IF(F.DIST.RT(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies F.DIST.RT to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with F.DIST.RT for aggregate calculations across multiple results:*
```spreadsheets
=SUM(F.DIST.RT(A1:A5),F.DIST.RT(B1:B5),F.DIST.RT(C1:C5))
```
This formula calculates F.DIST.RT for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with F.DIST.RT for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(F.DIST.RT(A1:A10))
```
This formula combines AVERAGE and F.DIST.RT for comprehensive data analysis
