...
T.TEST performs statistical analysis and calculations for data summarization and insights.

> [!f(x)] T.TEST Syntax
>
> ```spreadsheets
> T.TEST(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> T.TEST(A1) → result // Basic calculation
> 
> T.TEST(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with T.TEST for conditional logic and decision making:*
```spreadsheets
=IF(T.TEST(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies T.TEST to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with T.TEST for aggregate calculations across multiple results:*
```spreadsheets
=SUM(T.TEST(A1:A5),T.TEST(B1:B5),T.TEST(C1:C5))
```
This formula calculates T.TEST for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with T.TEST for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(T.TEST(A1:A10))
```
This formula combines AVERAGE and T.TEST for comprehensive data analysis
