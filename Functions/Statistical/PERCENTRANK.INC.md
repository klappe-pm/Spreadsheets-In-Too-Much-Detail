...
## PERCENTRANK.INC Description

PERCENTRANK.INC performs statistical analysis and calculations for data summarization and insights.

> [!f(x)] PERCENTRANK.INC Syntax
>
> ```spreadsheets
> PERCENTRANK.INC(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> PERCENTRANK.INC(A1) → result // Basic calculation
> 
> PERCENTRANK.INC(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with PERCENTRANK.INC for conditional logic and decision making:*
```spreadsheets
=IF(PERCENTRANK.INC(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies PERCENTRANK.INC to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with PERCENTRANK.INC for aggregate calculations across multiple results:*
```spreadsheets
=SUM(PERCENTRANK.INC(A1:A5),PERCENTRANK.INC(B1:B5),PERCENTRANK.INC(C1:C5))
```
This formula calculates PERCENTRANK.INC for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with PERCENTRANK.INC for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(PERCENTRANK.INC(A1:A10))
```
This formula combines AVERAGE and PERCENTRANK.INC for comprehensive data analysis
