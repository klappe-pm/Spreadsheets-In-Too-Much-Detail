...
## GAMMA.DIST Description

GAMMA.DIST performs statistical analysis and calculations for data summarization and insights.

> [!f(x)] GAMMA.DIST Syntax
>
> ```spreadsheets
> GAMMA.DIST(input_value, [options])
> ```
>
> **Parameters:**
...
> [!f(x)] GAMMA.DIST Examples
>
> ```spreadsheets
> GAMMA.DIST(A1) → result // Basic calculation
> 
> GAMMA.DIST(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with GAMMA.DIST for conditional logic and decision making:*
```spreadsheets
=IF(GAMMA.DIST(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies GAMMA.DIST to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with GAMMA.DIST for aggregate calculations across multiple results:*
```spreadsheets
=SUM(GAMMA.DIST(A1:A5),GAMMA.DIST(B1:B5),GAMMA.DIST(C1:C5))
```
This formula calculates GAMMA.DIST for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with GAMMA.DIST for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(GAMMA.DIST(A1:A10))
```
This formula combines AVERAGE and GAMMA.DIST for comprehensive data analysis
