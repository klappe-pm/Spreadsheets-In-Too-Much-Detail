...
PERCENTILE.INC performs statistical analysis and calculations for data summarization and insights.

> [!f(x)] PERCENTILE.INC Syntax
>
> ```spreadsheets
> PERCENTILE.INC(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> PERCENTILE.INC(A1) → result // Basic calculation
> 
> PERCENTILE.INC(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with PERCENTILE.INC for conditional logic and decision making:*
```spreadsheets
=IF(PERCENTILE.INC(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies PERCENTILE.INC to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with PERCENTILE.INC for aggregate calculations across multiple results:*
```spreadsheets
=SUM(PERCENTILE.INC(A1:A5),PERCENTILE.INC(B1:B5),PERCENTILE.INC(C1:C5))
```
This formula calculates PERCENTILE.INC for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with PERCENTILE.INC for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(PERCENTILE.INC(A1:A10))
```
This formula combines AVERAGE and PERCENTILE.INC for comprehensive data analysis
