...
VAR performs statistical analysis and calculations for data summarization and insights.

> [!f(x)] VAR Syntax
>
> ```spreadsheets
> VAR(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> VAR(A1) → result // Basic calculation
> 
> VAR(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with VAR for conditional logic and decision making:*
```spreadsheets
=IF(VAR(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies VAR to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with VAR for aggregate calculations across multiple results:*
```spreadsheets
=SUM(VAR(A1:A5),VAR(B1:B5),VAR(C1:C5))
```
This formula calculates VAR for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with VAR for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(VAR(A1:A10))
```
This formula combines AVERAGE and VAR for comprehensive data analysis
