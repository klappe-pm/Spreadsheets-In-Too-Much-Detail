...
## HYPGEOM.DIST Description

HYPGEOM.DIST performs statistical analysis and calculations for data summarization and insights.

> [!f(x)] HYPGEOM.DIST Syntax
>
> ```spreadsheets
> HYPGEOM.DIST(input_value, [options])
> ```
>
> **Parameters:**
...
> [!f(x)] HYPGEOM.DIST Examples
>
> ```spreadsheets
> HYPGEOM.DIST(A1) → result // Basic calculation
> 
> HYPGEOM.DIST(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with HYPGEOM.DIST for conditional logic and decision making:*
```spreadsheets
=IF(HYPGEOM.DIST(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies HYPGEOM.DIST to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with HYPGEOM.DIST for aggregate calculations across multiple results:*
```spreadsheets
=SUM(HYPGEOM.DIST(A1:A5),HYPGEOM.DIST(B1:B5),HYPGEOM.DIST(C1:C5))
```
This formula calculates HYPGEOM.DIST for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with HYPGEOM.DIST for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(HYPGEOM.DIST(A1:A10))
```
This formula combines AVERAGE and HYPGEOM.DIST for comprehensive data analysis
