...
MEDIAN performs statistical analysis and calculations for data summarization and insights.

> [!f(x)] MEDIAN Syntax
>
> ```spreadsheets
> MEDIAN(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> MEDIAN(A1) → result // Basic calculation
> 
> MEDIAN(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with MEDIAN for conditional logic and decision making:*
```spreadsheets
=IF(MEDIAN(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies MEDIAN to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with MEDIAN for aggregate calculations across multiple results:*
```spreadsheets
=SUM(MEDIAN(A1:A5),MEDIAN(B1:B5),MEDIAN(C1:C5))
```
This formula calculates MEDIAN for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with MEDIAN for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(MEDIAN(A1:A10))
```
This formula combines AVERAGE and MEDIAN for comprehensive data analysis
