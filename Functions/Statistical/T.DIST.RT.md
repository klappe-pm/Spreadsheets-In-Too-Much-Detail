...
T.DIST.RT performs statistical analysis and calculations for data summarization and insights.

> [!f(x)] T.DIST.RT Syntax
>
> ```spreadsheets
> T.DIST.RT(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> T.DIST.RT(A1) → result // Basic calculation
> 
> T.DIST.RT(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with T.DIST.RT for conditional logic and decision making:*
```spreadsheets
=IF(T.DIST.RT(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies T.DIST.RT to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with T.DIST.RT for aggregate calculations across multiple results:*
```spreadsheets
=SUM(T.DIST.RT(A1:A5),T.DIST.RT(B1:B5),T.DIST.RT(C1:C5))
```
This formula calculates T.DIST.RT for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with T.DIST.RT for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(T.DIST.RT(A1:A10))
```
This formula combines AVERAGE and T.DIST.RT for comprehensive data analysis
