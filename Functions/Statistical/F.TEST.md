...
F.TEST performs statistical analysis and calculations for data summarization and insights.

> [!f(x)] F.TEST Syntax
>
> ```spreadsheets
> F.TEST(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> F.TEST(A1) → result // Basic calculation
> 
> F.TEST(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with F.TEST for conditional logic and decision making:*
```spreadsheets
=IF(F.TEST(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies F.TEST to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with F.TEST for aggregate calculations across multiple results:*
```spreadsheets
=SUM(F.TEST(A1:A5),F.TEST(B1:B5),F.TEST(C1:C5))
```
This formula calculates F.TEST for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with F.TEST for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(F.TEST(A1:A10))
```
This formula combines AVERAGE and F.TEST for comprehensive data analysis
