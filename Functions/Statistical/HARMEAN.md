...
HARMEAN performs statistical analysis and calculations for data summarization and insights.

> [!f(x)] HARMEAN Syntax
>
> ```spreadsheets
> HARMEAN(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> HARMEAN(A1) → result // Basic calculation
> 
> HARMEAN(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with HARMEAN for conditional logic and decision making:*
```spreadsheets
=IF(HARMEAN(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies HARMEAN to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with HARMEAN for aggregate calculations across multiple results:*
```spreadsheets
=SUM(HARMEAN(A1:A5),HARMEAN(B1:B5),HARMEAN(C1:C5))
```
This formula calculates HARMEAN for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with HARMEAN for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(HARMEAN(A1:A10))
```
This formula combines AVERAGE and HARMEAN for comprehensive data analysis
