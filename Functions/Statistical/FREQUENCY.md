...
FREQUENCY performs statistical analysis and calculations for data summarization and insights.

> [!f(x)] FREQUENCY Syntax
>
> ```spreadsheets
> FREQUENCY(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> FREQUENCY(A1) → result // Basic calculation
> 
> FREQUENCY(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with FREQUENCY for conditional logic and decision making:*
```spreadsheets
=IF(FREQUENCY(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies FREQUENCY to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with FREQUENCY for aggregate calculations across multiple results:*
```spreadsheets
=SUM(FREQUENCY(A1:A5),FREQUENCY(B1:B5),FREQUENCY(C1:C5))
```
This formula calculates FREQUENCY for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with FREQUENCY for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(FREQUENCY(A1:A10))
```
This formula combines AVERAGE and FREQUENCY for comprehensive data analysis
