...
LOGNORM.DIST performs statistical analysis and calculations for data summarization and insights.

> [!f(x)] LOGNORM.DIST Syntax
>
> ```spreadsheets
> LOGNORM.DIST(input_value, [options])
> ```
>
> **Parameters:**
...
> [!f(x)] LOGNORM.DIST Examples
>
> ```spreadsheets
> LOGNORM.DIST(A1) → result // Basic calculation
> 
> LOGNORM.DIST(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with LOGNORM.DIST for conditional logic and decision making:*
```spreadsheets
=IF(LOGNORM.DIST(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies LOGNORM.DIST to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with LOGNORM.DIST for aggregate calculations across multiple results:*
```spreadsheets
=SUM(LOGNORM.DIST(A1:A5),LOGNORM.DIST(B1:B5),LOGNORM.DIST(C1:C5))
```
This formula calculates LOGNORM.DIST for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with LOGNORM.DIST for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(LOGNORM.DIST(A1:A10))
```
This formula combines AVERAGE and LOGNORM.DIST for comprehensive data analysis
