...
## FORECAST.ETS.CONFINT Description

FORECAST.ETS.CONFINT performs statistical analysis and calculations for data summarization and insights.

> [!f(x)] FORECAST.ETS.CONFINT Syntax
>
> ```spreadsheets
> FORECAST.ETS.CONFINT(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> FORECAST.ETS.CONFINT(A1) → result // Basic calculation
> 
> FORECAST.ETS.CONFINT(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with FORECAST.ETS.CONFINT for conditional logic and decision making:*
```spreadsheets
=IF(FORECAST.ETS.CONFINT(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies FORECAST.ETS.CONFINT to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with FORECAST.ETS.CONFINT for aggregate calculations across multiple results:*
```spreadsheets
=SUM(FORECAST.ETS.CONFINT(A1:A5),FORECAST.ETS.CONFINT(B1:B5),FORECAST.ETS.CONFINT(C1:C5))
```
This formula calculates FORECAST.ETS.CONFINT for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with FORECAST.ETS.CONFINT for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(FORECAST.ETS.CONFINT(A1:A10))
```
This formula combines AVERAGE and FORECAST.ETS.CONFINT for comprehensive data analysis
