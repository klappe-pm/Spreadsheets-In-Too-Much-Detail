...
STDEVA performs statistical analysis and calculations for data summarization and insights.

> [!f(x)] STDEVA Syntax
>
> ```spreadsheets
> STDEVA(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> STDEVA(A1) → result // Basic calculation
> 
> STDEVA(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with STDEVA for conditional logic and decision making:*
```spreadsheets
=IF(STDEVA(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies STDEVA to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with STDEVA for aggregate calculations across multiple results:*
```spreadsheets
=SUM(STDEVA(A1:A5),STDEVA(B1:B5),STDEVA(C1:C5))
```
This formula calculates STDEVA for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with STDEVA for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(STDEVA(A1:A10))
```
This formula combines AVERAGE and STDEVA for comprehensive data analysis
