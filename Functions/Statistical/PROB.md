...
PROB performs statistical analysis and calculations for data summarization and insights.

> [!f(x)] PROB Syntax
>
> ```spreadsheets
> PROB(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> PROB(A1) → result // Basic calculation
> 
> PROB(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with PROB for conditional logic and decision making:*
```spreadsheets
=IF(PROB(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies PROB to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with PROB for aggregate calculations across multiple results:*
```spreadsheets
=SUM(PROB(A1:A5),PROB(B1:B5),PROB(C1:C5))
```
This formula calculates PROB for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with PROB for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(PROB(A1:A10))
```
This formula combines AVERAGE and PROB for comprehensive data analysis
