...
## POISSON.DIST Description

POISSON.DIST performs statistical analysis and calculations for data summarization and insights.

> [!f(x)] POISSON.DIST Syntax
>
> ```spreadsheets
> POISSON.DIST(input_value, [options])
> ```
>
> **Parameters:**
...
> [!f(x)] POISSON.DIST Examples
>
> ```spreadsheets
> POISSON.DIST(A1) → result // Basic calculation
> 
> POISSON.DIST(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with POISSON.DIST for conditional logic and decision making:*
```spreadsheets
=IF(POISSON.DIST(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies POISSON.DIST to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with POISSON.DIST for aggregate calculations across multiple results:*
```spreadsheets
=SUM(POISSON.DIST(A1:A5),POISSON.DIST(B1:B5),POISSON.DIST(C1:C5))
```
This formula calculates POISSON.DIST for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with POISSON.DIST for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(POISSON.DIST(A1:A10))
```
This formula combines AVERAGE and POISSON.DIST for comprehensive data analysis
