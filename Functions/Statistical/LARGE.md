...
LARGE performs statistical analysis and calculations for data summarization and insights.

> [!f(x)] LARGE Syntax
>
> ```spreadsheets
> LARGE(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> LARGE(A1) → result // Basic calculation
> 
> LARGE(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with LARGE for conditional logic and decision making:*
```spreadsheets
=IF(LARGE(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies LARGE to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with LARGE for aggregate calculations across multiple results:*
```spreadsheets
=SUM(LARGE(A1:A5),LARGE(B1:B5),LARGE(C1:C5))
```
This formula calculates LARGE for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with LARGE for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(LARGE(A1:A10))
```
This formula combines AVERAGE and LARGE for comprehensive data analysis
