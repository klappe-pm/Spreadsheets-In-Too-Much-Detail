...
F.DIST performs statistical analysis and calculations for data summarization and insights.

> [!f(x)] F.DIST Syntax
>
> ```spreadsheets
> F.DIST(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> F.DIST(A1) → result // Basic calculation
> 
> F.DIST(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with F.DIST for conditional logic and decision making:*
```spreadsheets
=IF(F.DIST(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies F.DIST to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with F.DIST for aggregate calculations across multiple results:*
```spreadsheets
=SUM(F.DIST(A1:A5),F.DIST(B1:B5),F.DIST(C1:C5))
```
This formula calculates F.DIST for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with F.DIST for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(F.DIST(A1:A10))
```
This formula combines AVERAGE and F.DIST for comprehensive data analysis
