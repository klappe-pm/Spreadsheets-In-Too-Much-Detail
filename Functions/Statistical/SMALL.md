...
SMALL performs statistical analysis and calculations for data summarization and insights.

> [!f(x)] SMALL Syntax
>
> ```spreadsheets
> SMALL(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> SMALL(A1) → result // Basic calculation
> 
> SMALL(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with SMALL for conditional logic and decision making:*
```spreadsheets
=IF(SMALL(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies SMALL to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with SMALL for aggregate calculations across multiple results:*
```spreadsheets
=SUM(SMALL(A1:A5),SMALL(B1:B5),SMALL(C1:C5))
```
This formula calculates SMALL for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with SMALL for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(SMALL(A1:A10))
```
This formula combines AVERAGE and SMALL for comprehensive data analysis
