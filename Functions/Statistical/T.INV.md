...
T.INV performs statistical analysis and calculations for data summarization and insights.

> [!f(x)] T.INV Syntax
>
> ```spreadsheets
> T.INV(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> T.INV(A1) → result // Basic calculation
> 
> T.INV(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with T.INV for conditional logic and decision making:*
```spreadsheets
=IF(T.INV(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies T.INV to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with T.INV for aggregate calculations across multiple results:*
```spreadsheets
=SUM(T.INV(A1:A5),T.INV(B1:B5),T.INV(C1:C5))
```
This formula calculates T.INV for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with T.INV for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(T.INV(A1:A10))
```
This formula combines AVERAGE and T.INV for comprehensive data analysis
