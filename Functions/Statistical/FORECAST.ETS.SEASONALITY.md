...
## FORECAST.ETS.SEASONALITY Description

FORECAST.ETS.SEASONALITY performs statistical analysis and calculations for data summarization and insights.

> [!f(x)] FORECAST.ETS.SEASONALITY Syntax
>
> ```spreadsheets
> FORECAST.ETS.SEASONALITY(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> FORECAST.ETS.SEASONALITY(A1) → result // Basic calculation
> 
> FORECAST.ETS.SEASONALITY(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with FORECAST.ETS.SEASONALITY for conditional logic and decision making:*
```spreadsheets
=IF(FORECAST.ETS.SEASONALITY(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies FORECAST.ETS.SEASONALITY to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with FORECAST.ETS.SEASONALITY for aggregate calculations across multiple results:*
```spreadsheets
=SUM(FORECAST.ETS.SEASONALITY(A1:A5),FORECAST.ETS.SEASONALITY(B1:B5),FORECAST.ETS.SEASONALITY(C1:C5))
```
This formula calculates FORECAST.ETS.SEASONALITY for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with FORECAST.ETS.SEASONALITY for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(FORECAST.ETS.SEASONALITY(A1:A10))
```
This formula combines AVERAGE and FORECAST.ETS.SEASONALITY for comprehensive data analysis
