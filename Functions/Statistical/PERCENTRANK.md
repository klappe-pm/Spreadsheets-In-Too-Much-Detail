...
## PERCENTRANK Description

PERCENTRANK performs statistical analysis and calculations for data summarization and insights.

> [!f(x)] PERCENTRANK Syntax
>
> ```spreadsheets
> PERCENTRANK(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> PERCENTRANK(A1) → result // Basic calculation
> 
> PERCENTRANK(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with PERCENTRANK for conditional logic and decision making:*
```spreadsheets
=IF(PERCENTRANK(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies PERCENTRANK to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with PERCENTRANK for aggregate calculations across multiple results:*
```spreadsheets
=SUM(PERCENTRANK(A1:A5),PERCENTRANK(B1:B5),PERCENTRANK(C1:C5))
```
This formula calculates PERCENTRANK for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with PERCENTRANK for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(PERCENTRANK(A1:A10))
```
This formula combines AVERAGE and PERCENTRANK for comprehensive data analysis
