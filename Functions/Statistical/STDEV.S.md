...
STDEV.S performs statistical analysis and calculations for data summarization and insights.

> [!f(x)] STDEV.S Syntax
>
> ```spreadsheets
> STDEV.S(input_value, [options])
> ```
>
> **Parameters:**
...
> [!f(x)] STDEV.S Examples
>
> ```spreadsheets
> STDEV.S(A1) → result // Basic calculation
> 
> STDEV.S(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with STDEV.S for conditional logic and decision making:*
```spreadsheets
=IF(STDEV.S(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies STDEV.S to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with STDEV.S for aggregate calculations across multiple results:*
```spreadsheets
=SUM(STDEV.S(A1:A5),STDEV.S(B1:B5),STDEV.S(C1:C5))
```
This formula calculates STDEV.S for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with STDEV.S for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(STDEV.S(A1:A10))
```
This formula combines AVERAGE and STDEV.S for comprehensive data analysis
