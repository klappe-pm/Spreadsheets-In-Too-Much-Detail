...
LOOKUP performs specialized calculations for analytical applications.

> [!f(x)] LOOKUP Syntax
>
> ```spreadsheets
> LOOKUP(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> LOOKUP(A1) → result // Basic calculation
> 
> LOOKUP(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with LOOKUP for conditional logic and decision making:*
```spreadsheets
=IF(LOOKUP(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies LOOKUP to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with LOOKUP for aggregate calculations across multiple results:*
```spreadsheets
=SUM(LOOKUP(A1:A5),LOOKUP(B1:B5),LOOKUP(C1:C5))
```
This formula calculates LOOKUP for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with LOOKUP for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(LOOKUP(A1:A10))
```
This formula combines AVERAGE and LOOKUP for comprehensive data analysis
