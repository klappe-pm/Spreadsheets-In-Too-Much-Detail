...
TEXTSPLIT manipulates and analyzes text strings for data processing and formatting.

> [!f(x)] TEXTSPLIT Syntax
>
> ```spreadsheets
> TEXTSPLIT(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> TEXTSPLIT(A1) → result // Basic calculation
> 
> TEXTSPLIT(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with TEXTSPLIT for conditional logic and decision making:*
```spreadsheets
=IF(TEXTSPLIT(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies TEXTSPLIT to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with TEXTSPLIT for aggregate calculations across multiple results:*
```spreadsheets
=SUM(TEXTSPLIT(A1:A5),TEXTSPLIT(B1:B5),TEXTSPLIT(C1:C5))
```
This formula calculates TEXTSPLIT for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with TEXTSPLIT for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(TEXTSPLIT(A1:A10))
```
This formula combines AVERAGE and TEXTSPLIT for comprehensive data analysis
