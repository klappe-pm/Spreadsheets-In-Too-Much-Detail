...
CLEAN manipulates and analyzes text strings for data processing and formatting.

> [!f(x)] CLEAN Syntax
>
> ```spreadsheets
> CLEAN(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> CLEAN(A1) → result // Basic calculation
> 
> CLEAN(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with CLEAN for conditional logic and decision making:*
```spreadsheets
=IF(CLEAN(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies CLEAN to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with CLEAN for aggregate calculations across multiple results:*
```spreadsheets
=SUM(CLEAN(A1:A5),CLEAN(B1:B5),CLEAN(C1:C5))
```
This formula calculates CLEAN for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with CLEAN for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(CLEAN(A1:A10))
```
This formula combines AVERAGE and CLEAN for comprehensive data analysis
