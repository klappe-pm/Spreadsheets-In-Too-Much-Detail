...
DOLLAR manipulates and analyzes text strings for data processing and formatting.

> [!f(x)] DOLLAR Syntax
>
> ```spreadsheets
> DOLLAR(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> DOLLAR(A1) → result // Basic calculation
> 
> DOLLAR(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with DOLLAR for conditional logic and decision making:*
```spreadsheets
=IF(DOLLAR(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies DOLLAR to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with DOLLAR for aggregate calculations across multiple results:*
```spreadsheets
=SUM(DOLLAR(A1:A5),DOLLAR(B1:B5),DOLLAR(C1:C5))
```
This formula calculates DOLLAR for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with DOLLAR for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(DOLLAR(A1:A10))
```
This formula combines AVERAGE and DOLLAR for comprehensive data analysis
