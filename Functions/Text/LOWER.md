...
LOWER manipulates and analyzes text strings for data processing and formatting.

> [!f(x)] LOWER Syntax
>
> ```spreadsheets
> LOWER(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> LOWER(A1) → result // Basic calculation
> 
> LOWER(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with LOWER for conditional logic and decision making:*
```spreadsheets
=IF(LOWER(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies LOWER to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with LOWER for aggregate calculations across multiple results:*
```spreadsheets
=SUM(LOWER(A1:A5),LOWER(B1:B5),LOWER(C1:C5))
```
This formula calculates LOWER for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with LOWER for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(LOWER(A1:A10))
```
This formula combines AVERAGE and LOWER for comprehensive data analysis
