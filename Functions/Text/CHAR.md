...
CHAR manipulates and analyzes text strings for data processing and formatting.

> [!f(x)] CHAR Syntax
>
> ```spreadsheets
> CHAR(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> CHAR(A1) → result // Basic calculation
> 
> CHAR(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with CHAR for conditional logic and decision making:*
```spreadsheets
=IF(CHAR(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies CHAR to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with CHAR for aggregate calculations across multiple results:*
```spreadsheets
=SUM(CHAR(A1:A5),CHAR(B1:B5),CHAR(C1:C5))
```
This formula calculates CHAR for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with CHAR for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(CHAR(A1:A10))
```
This formula combines AVERAGE and CHAR for comprehensive data analysis
