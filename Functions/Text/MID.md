...
MID manipulates and analyzes text strings for data processing and formatting.

> [!f(x)] MID Syntax
>
> ```spreadsheets
> MID(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> MID(A1) → result // Basic calculation
> 
> MID(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with MID for conditional logic and decision making:*
```spreadsheets
=IF(MID(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies MID to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with MID for aggregate calculations across multiple results:*
```spreadsheets
=SUM(MID(A1:A5),MID(B1:B5),MID(C1:C5))
```
This formula calculates MID for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with MID for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(MID(A1:A10))
```
This formula combines AVERAGE and MID for comprehensive data analysis
