...
COUPNUM calculates financial metrics including payments, returns, and valuations.

> [!f(x)] COUPNUM Syntax
>
> ```spreadsheets
> COUPNUM(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> COUPNUM(A1) → result // Basic calculation
> 
> COUPNUM(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with COUPNUM for conditional logic and decision making:*
```spreadsheets
=IF(COUPNUM(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies COUPNUM to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with COUPNUM for aggregate calculations across multiple results:*
```spreadsheets
=SUM(COUPNUM(A1:A5),COUPNUM(B1:B5),COUPNUM(C1:C5))
```
This formula calculates COUPNUM for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with COUPNUM for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(COUPNUM(A1:A10))
```
This formula combines AVERAGE and COUPNUM for comprehensive data analysis
