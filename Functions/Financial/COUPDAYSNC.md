...
COUPDAYSNC calculates financial metrics including payments, returns, and valuations.

> [!f(x)] COUPDAYSNC Syntax
>
> ```spreadsheets
> COUPDAYSNC(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> COUPDAYSNC(A1) → result // Basic calculation
> 
> COUPDAYSNC(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with COUPDAYSNC for conditional logic and decision making:*
```spreadsheets
=IF(COUPDAYSNC(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies COUPDAYSNC to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with COUPDAYSNC for aggregate calculations across multiple results:*
```spreadsheets
=SUM(COUPDAYSNC(A1:A5),COUPDAYSNC(B1:B5),COUPDAYSNC(C1:C5))
```
This formula calculates COUPDAYSNC for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with COUPDAYSNC for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(COUPDAYSNC(A1:A10))
```
This formula combines AVERAGE and COUPDAYSNC for comprehensive data analysis
