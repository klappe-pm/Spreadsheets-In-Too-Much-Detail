...
FV calculates financial metrics including payments, returns, and valuations.

> [!f(x)] FV Syntax
>
> ```spreadsheets
> FV(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> FV(A1) → result // Basic calculation
> 
> FV(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with FV for conditional logic and decision making:*
```spreadsheets
=IF(FV(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies FV to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with FV for aggregate calculations across multiple results:*
```spreadsheets
=SUM(FV(A1:A5),FV(B1:B5),FV(C1:C5))
```
This formula calculates FV for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with FV for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(FV(A1:A10))
```
This formula combines AVERAGE and FV for comprehensive data analysis
