...
INTRATE calculates financial metrics including payments, returns, and valuations.

> [!f(x)] INTRATE Syntax
>
> ```spreadsheets
> INTRATE(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> INTRATE(A1) → result // Basic calculation
> 
> INTRATE(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with INTRATE for conditional logic and decision making:*
```spreadsheets
=IF(INTRATE(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies INTRATE to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with INTRATE for aggregate calculations across multiple results:*
```spreadsheets
=SUM(INTRATE(A1:A5),INTRATE(B1:B5),INTRATE(C1:C5))
```
This formula calculates INTRATE for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with INTRATE for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(INTRATE(A1:A10))
```
This formula combines AVERAGE and INTRATE for comprehensive data analysis
