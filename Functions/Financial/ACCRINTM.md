...
## ACCRINTM Description

ACCRINTM calculates financial metrics including payments, returns, and valuations.

> [!f(x)] ACCRINTM Syntax
>
> ```spreadsheets
> ACCRINTM(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> ACCRINTM(A1) → result // Basic calculation
> 
> ACCRINTM(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with ACCRINTM for conditional logic and decision making:*
```spreadsheets
=IF(ACCRINTM(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies ACCRINTM to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with ACCRINTM for aggregate calculations across multiple results:*
```spreadsheets
=SUM(ACCRINTM(A1:A5),ACCRINTM(B1:B5),ACCRINTM(C1:C5))
```
This formula calculates ACCRINTM for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with ACCRINTM for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(ACCRINTM(A1:A10))
```
This formula combines AVERAGE and ACCRINTM for comprehensive data analysis
