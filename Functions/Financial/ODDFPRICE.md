...
ODDFPRICE calculates financial metrics including payments, returns, and valuations.

> [!f(x)] ODDFPRICE Syntax
>
> ```spreadsheets
> ODDFPRICE(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> ODDFPRICE(A1) → result // Basic calculation
> 
> ODDFPRICE(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with ODDFPRICE for conditional logic and decision making:*
```spreadsheets
=IF(ODDFPRICE(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies ODDFPRICE to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with ODDFPRICE for aggregate calculations across multiple results:*
```spreadsheets
=SUM(ODDFPRICE(A1:A5),ODDFPRICE(B1:B5),ODDFPRICE(C1:C5))
```
This formula calculates ODDFPRICE for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with ODDFPRICE for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(ODDFPRICE(A1:A10))
```
This formula combines AVERAGE and ODDFPRICE for comprehensive data analysis
