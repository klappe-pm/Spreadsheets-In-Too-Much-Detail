...
ODDFYIELD calculates financial metrics including payments, returns, and valuations.

> [!f(x)] ODDFYIELD Syntax
>
> ```spreadsheets
> ODDFYIELD(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> ODDFYIELD(A1) → result // Basic calculation
> 
> ODDFYIELD(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with ODDFYIELD for conditional logic and decision making:*
```spreadsheets
=IF(ODDFYIELD(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies ODDFYIELD to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with ODDFYIELD for aggregate calculations across multiple results:*
```spreadsheets
=SUM(ODDFYIELD(A1:A5),ODDFYIELD(B1:B5),ODDFYIELD(C1:C5))
```
This formula calculates ODDFYIELD for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with ODDFYIELD for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(ODDFYIELD(A1:A10))
```
This formula combines AVERAGE and ODDFYIELD for comprehensive data analysis
