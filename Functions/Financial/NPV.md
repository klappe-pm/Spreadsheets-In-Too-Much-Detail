...
NPV calculates financial metrics including payments, returns, and valuations.

> [!f(x)] NPV Syntax
>
> ```spreadsheets
> NPV(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> NPV(A1) → result // Basic calculation
> 
> NPV(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with NPV for conditional logic and decision making:*
```spreadsheets
=IF(NPV(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies NPV to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with NPV for aggregate calculations across multiple results:*
```spreadsheets
=SUM(NPV(A1:A5),NPV(B1:B5),NPV(C1:C5))
```
This formula calculates NPV for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with NPV for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(NPV(A1:A10))
```
This formula combines AVERAGE and NPV for comprehensive data analysis
