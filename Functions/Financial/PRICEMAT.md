...
## PRICEMAT Description

PRICEMAT calculates financial metrics including payments, returns, and valuations.

> [!f(x)] PRICEMAT Syntax
>
> ```spreadsheets
> PRICEMAT(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> PRICEMAT(A1) → result // Basic calculation
> 
> PRICEMAT(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with PRICEMAT for conditional logic and decision making:*
```spreadsheets
=IF(PRICEMAT(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies PRICEMAT to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with PRICEMAT for aggregate calculations across multiple results:*
```spreadsheets
=SUM(PRICEMAT(A1:A5),PRICEMAT(B1:B5),PRICEMAT(C1:C5))
```
This formula calculates PRICEMAT for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with PRICEMAT for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(PRICEMAT(A1:A10))
```
This formula combines AVERAGE and PRICEMAT for comprehensive data analysis
