...
XIRR calculates financial metrics including payments, returns, and valuations.

> [!f(x)] XIRR Syntax
>
> ```spreadsheets
> XIRR(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> XIRR(A1) → result // Basic calculation
> 
> XIRR(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with XIRR for conditional logic and decision making:*
```spreadsheets
=IF(XIRR(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies XIRR to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with XIRR for aggregate calculations across multiple results:*
```spreadsheets
=SUM(XIRR(A1:A5),XIRR(B1:B5),XIRR(C1:C5))
```
This formula calculates XIRR for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with XIRR for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(XIRR(A1:A10))
```
This formula combines AVERAGE and XIRR for comprehensive data analysis
