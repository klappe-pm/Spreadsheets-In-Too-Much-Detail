...
DURATION calculates financial metrics including payments, returns, and valuations.

> [!f(x)] DURATION Syntax
>
> ```spreadsheets
> DURATION(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> DURATION(A1) → result // Basic calculation
> 
> DURATION(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with DURATION for conditional logic and decision making:*
```spreadsheets
=IF(DURATION(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies DURATION to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with DURATION for aggregate calculations across multiple results:*
```spreadsheets
=SUM(DURATION(A1:A5),DURATION(B1:B5),DURATION(C1:C5))
```
This formula calculates DURATION for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with DURATION for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(DURATION(A1:A10))
```
This formula combines AVERAGE and DURATION for comprehensive data analysis
