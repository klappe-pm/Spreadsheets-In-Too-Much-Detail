...
YIELDDISC calculates financial metrics including payments, returns, and valuations.

> [!f(x)] YIELDDISC Syntax
>
> ```spreadsheets
> YIELDDISC(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> YIELDDISC(A1) → result // Basic calculation
> 
> YIELDDISC(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with YIELDDISC for conditional logic and decision making:*
```spreadsheets
=IF(YIELDDISC(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies YIELDDISC to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with YIELDDISC for aggregate calculations across multiple results:*
```spreadsheets
=SUM(YIELDDISC(A1:A5),YIELDDISC(B1:B5),YIELDDISC(C1:C5))
```
This formula calculates YIELDDISC for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with YIELDDISC for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(YIELDDISC(A1:A10))
```
This formula combines AVERAGE and YIELDDISC for comprehensive data analysis
