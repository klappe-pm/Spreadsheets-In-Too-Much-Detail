...
## MDURATION Description

MDURATION calculates financial metrics including payments, returns, and valuations.

> [!f(x)] MDURATION Syntax
>
> ```spreadsheets
> MDURATION(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> MDURATION(A1) → result // Basic calculation
> 
> MDURATION(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with MDURATION for conditional logic and decision making:*
```spreadsheets
=IF(MDURATION(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies MDURATION to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with MDURATION for aggregate calculations across multiple results:*
```spreadsheets
=SUM(MDURATION(A1:A5),MDURATION(B1:B5),MDURATION(C1:C5))
```
This formula calculates MDURATION for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with MDURATION for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(MDURATION(A1:A10))
```
This formula combines AVERAGE and MDURATION for comprehensive data analysis
