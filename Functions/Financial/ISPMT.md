...
ISPMT calculates financial metrics including payments, returns, and valuations.

> [!f(x)] ISPMT Syntax
>
> ```spreadsheets
> ISPMT(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> ISPMT(A1) → result // Basic calculation
> 
> ISPMT(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with ISPMT for conditional logic and decision making:*
```spreadsheets
=IF(ISPMT(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies ISPMT to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with ISPMT for aggregate calculations across multiple results:*
```spreadsheets
=SUM(ISPMT(A1:A5),ISPMT(B1:B5),ISPMT(C1:C5))
```
This formula calculates ISPMT for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with ISPMT for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(ISPMT(A1:A10))
```
This formula combines AVERAGE and ISPMT for comprehensive data analysis
