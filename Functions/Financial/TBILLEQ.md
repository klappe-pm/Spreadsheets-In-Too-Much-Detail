...
TBILLEQ calculates financial metrics including payments, returns, and valuations.

> [!f(x)] TBILLEQ Syntax
>
> ```spreadsheets
> TBILLEQ(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> TBILLEQ(A1) → result // Basic calculation
> 
> TBILLEQ(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with TBILLEQ for conditional logic and decision making:*
```spreadsheets
=IF(TBILLEQ(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies TBILLEQ to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with TBILLEQ for aggregate calculations across multiple results:*
```spreadsheets
=SUM(TBILLEQ(A1:A5),TBILLEQ(B1:B5),TBILLEQ(C1:C5))
```
This formula calculates TBILLEQ for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with TBILLEQ for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(TBILLEQ(A1:A10))
```
This formula combines AVERAGE and TBILLEQ for comprehensive data analysis
