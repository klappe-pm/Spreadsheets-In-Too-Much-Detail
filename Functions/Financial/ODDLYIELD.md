...
## ODDLYIELD Description

ODDLYIELD calculates financial metrics including payments, returns, and valuations.

> [!f(x)] ODDLYIELD Syntax
>
> ```spreadsheets
> ODDLYIELD(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> ODDLYIELD(A1) → result // Basic calculation
> 
> ODDLYIELD(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with ODDLYIELD for conditional logic and decision making:*
```spreadsheets
=IF(ODDLYIELD(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies ODDLYIELD to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with ODDLYIELD for aggregate calculations across multiple results:*
```spreadsheets
=SUM(ODDLYIELD(A1:A5),ODDLYIELD(B1:B5),ODDLYIELD(C1:C5))
```
This formula calculates ODDLYIELD for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with ODDLYIELD for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(ODDLYIELD(A1:A10))
```
This formula combines AVERAGE and ODDLYIELD for comprehensive data analysis
