...
PRICE calculates financial metrics including payments, returns, and valuations.

> [!f(x)] PRICE Syntax
>
> ```spreadsheets
> PRICE(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> PRICE(A1) → result // Basic calculation
> 
> PRICE(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with PRICE for conditional logic and decision making:*
```spreadsheets
=IF(PRICE(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies PRICE to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with PRICE for aggregate calculations across multiple results:*
```spreadsheets
=SUM(PRICE(A1:A5),PRICE(B1:B5),PRICE(C1:C5))
```
This formula calculates PRICE for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with PRICE for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(PRICE(A1:A10))
```
This formula combines AVERAGE and PRICE for comprehensive data analysis
