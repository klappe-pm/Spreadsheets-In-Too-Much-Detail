...
MIRR calculates financial metrics including payments, returns, and valuations.

> [!f(x)] MIRR Syntax
>
> ```spreadsheets
> MIRR(input_value, [options])
> ```
>
> **Parameters:**
...
>
> ```spreadsheets
> MIRR(A1) → result // Basic calculation
> 
> MIRR(A1:A10) → range_result // Process entire range
> ```
...
*Use IF with MIRR for conditional logic and decision making:*
```spreadsheets
=IF(MIRR(A1:A10)>threshold_value,"Condition Met","Condition Not Met")
```
This formula applies MIRR to a range and compares the result to a threshold, returning different text based on the condition

**[[SUM]]** - Aggregate values for total calculations

*Use SUM with MIRR for aggregate calculations across multiple results:*
```spreadsheets
=SUM(MIRR(A1:A5),MIRR(B1:B5),MIRR(C1:C5))
```
This formula calculates MIRR for multiple ranges and sums the results together

**[[AVERAGE]]** - Calculate arithmetic mean for central tendency analysis

*Use AVERAGE with MIRR for enhanced analytical workflows:*
```spreadsheets
=AVERAGE(MIRR(A1:A10))
```
This formula combines AVERAGE and MIRR for comprehensive data analysis
