...
// node_modules/obsidian-smart-env/src/utils/onboarding_events_utils.js
var DEFAULT_IDLE_DELAY_MS = 3e3;
function update_visibility_idle_state(params = {}) {
...
function get_idle_delay_ms(params = {}) {
  const {
    last_input_at = 0,
    now = Date.now(),
...
function enqueue_event_key(queue, params = {}) {
...
function dequeue_event_key(queue) {
...
// node_modules/obsidian-smart-env/src/utils/onboarding_events.js
var MILESTONE_IDLE_DELAY_MS = DEFAULT_IDLE_DELAY_MS;
var MILESTONE_NOTICE_DURATION_MS = 7e3;
var MILESTONE_INPUT_EVENTS = [
  "keydown",
  "mousedown",
  "mousemove",
  "touchstart",
...
function register_first_of_event_notifications(env) {
  ...
  const show_milestone_notice = (event_key) => {
    ...
    new import_obsidian13.Notice(frag, MILESTONE_NOTICE_DURATION_MS);
...
var notification_levels = ["attention", "error", "warning", "info"];
...
function render_filter_controls(container, params = {}) {
  if (!container) return;
  const { active_levels = /* @__PURE__ */ new Set(), level_counts = {}, on_change = () => {
  } } = params;
...
function append_entry(feed_container, entry) {
  ...
  const row = has_payload ? feed_container.ownerDocument.createElement("details") : feed_container.ownerDocument.createElement("div");
  row.className = "smart-env-notification";
  row.dataset.level = level;
  row.setAttribute("role", "listitem");
  const summary = has_payload ? feed_container.ownerDocument.createElement("summary") : feed_container.ownerDocument.createElement("div");
...
function get_entry_payload_text(entry) {
  ...
  return Object.entries(event_obj).filter(([k]) => !["at", "collection_key"].includes(k)).map(([k, v]) => `  ${k}: ${typeof v === "string" ? v : JSON.stringify(v)}`).join("\n");
}
...
var SettingGroupPolyfill = class {
  ...
  addSetting(callback) {
    const setting = new import_obsidian18.Setting(this.listEl);
          ...
          setting.addToggle((toggle) => {
            ...
            toggle.onChange((value) => {
              if (settng_is_pro && !env_is_pro) {
                new import_obsidian18.Notice("Nice try! This is a PRO feature. Please upgrade to access this setting.");
                return;
              }
...
function render_heading_button(setting_group, scope, heading_btn) {
  ...
  if (heading_btn.btn_icon) {
    (0, import_obsidian18.setIcon)(btn_el, heading_btn.btn_icon);
  }
...
// node_modules/obsidian-smart-env/src/modals/smart_model_modal.js
var SmartModelModal = class extends import_obsidian19.Modal {
  /**
   * @param {App} app
...
async function post_process10(model, container, params) {
  ...
  (0, import_obsidian20.setIcon)(icon_el, get_test_result_icon_name(model));
...
function add_named_context_items(ctx, params = {}) {
  const payloads = Array.isArray(params.payloads) ? params.payloads : build_named_context_item_payloads(ctx, params);
  if (!payloads.length) return [];
  if (is_codeblock_context(ctx)) {
    ...
    suggestions.push({
      ...
      mod_select_action: ({ modal: modal2 } = {}) => {
        ...
        if (modal2?.setInstructions) {
          const purpose = payloads.length ? `Added ${payloads.length} item(s) from ${other_name}` : `No items to add from ${other_name}`;
...
var MOD_CHAR2 = import_obsidian34.Platform.isMacOS ? "\u2318" : "Ctrl";
...
async function open_source(item, event = null) {
  try {
    ...
    let leaf;
    if (event) {
      const is_mod = import_obsidian35.Keymap.isModEvent(event);
      const is_alt = import_obsidian35.Keymap.isModifier(event, "Alt");
      if (is_mod && is_alt) {
  ...
  modules: {
    ...
    smart_chat_model: {
      ...
      http_adapter: new SmartHttpRequest({
        ...
        obsidian_request_url: import_obsidian38.requestUrl
      })
    },
    http_adapter: {
...
function add_smart_chat_icon() {
  (0, import_obsidian39.addIcon)("smart-chat", `<defs>
...
function add_smart_connections_icon() {
  (0, import_obsidian39.addIcon)("smart-connections", `<path d="M50,20 L80,40 L80,60 L50,100" stroke="currentColor" stroke-width="4" fill="none"/>
...
function add_smart_lookup_icon() {
  (0, import_obsidian39.addIcon)("smart-lookup", `<defs>
...
function add_smart_copy_context_icon() {
  (0, import_obsidian39.addIcon)("smart-copy-note", smart_copy_note_svg);
}
function add_smart_context_icon() {
  (0, import_obsidian39.addIcon)("smart-context-builder", smart_context_builder_svg);
}
function add_inline_connections_icon() {
  (0, import_obsidian39.addIcon)("smart-inline-connections", smart_inline_connections_svg);
}
...
function add_footer_connections_icon() {
  (0, import_obsidian39.addIcon)("smart-footer-connections", smart_footer_connections_svg);
}
function add_smart_dupe_detector_icon() {
  (0, import_obsidian39.addIcon)("smart-dupe-detector", `
...
function add_smart_named_contexts_icon() {
  (0, import_obsidian39.addIcon)("smart-named-contexts", `
...
var SmartNotices = class {
  ...
  _add_mute_button(id, container) {
    const btn = document.createElement("button");
    (0, import_obsidian40.setIcon)(btn, "bell-off");
    btn.addEventListener("click", () => {
...
var SmartEnv2 = class extends SmartEnv {
  ...
  static async create(plugin, env_config) {
    ...
    if (window.smart_env && !window.smart_env.constructor.version) {
      ...
      new import_obsidian43.Notice(update_notice, 0);
  ...
  async load(force_load = false) {
    ...
    if (import_obsidian43.Platform.isMobile && !force_load) {
      ...
      new import_obsidian43.Notice(frag, 0);
      return;
    }
    await super.load();
  ...
  get notices() {
    if (!this._notices) {
      this._notices = new SmartNotices(this, {
        adapter: import_obsidian43.Notice
      });
    }
    return this._notices;
  }
  ...
  async handle_smart_plugins_oauth_callback(params) {
    ...
    if (!code) {
      new import_obsidian43.Notice("No OAuth code provided in URL. Login failed.");
      return;
    }
    try {
      ...
      new import_obsidian43.Notice(`OAuth callback error: ${err.message}`);
    }
  }
  /**
...
var DEFAULT_LINK_REGEX = /(https?:\/\/[^\s]+)/g;
var MARKDOWN_LINK_REGEX = /\((https?:\/\/[^)\s]+)\)/g;
var URL_TRAILING_CHARS_REGEX = /[)\].,>;:"']+$/;
var URL_LEADING_CHARS_REGEX = /^[<(]+/;
function strip_wrapping_url_chars(url_value) {
...
function extract_urls_from_line({ line, link_regex }) {
...
function line_contains_url({ line, target_url, link_regex }) {
  ...
  return candidates.some((candidate) => {
    ...
    return candidate === target_url || candidate === normalized_target_url || normalized_candidate === normalized_target_url;
  });
}
...
function resolve_add_thread_button_anchor({
...
function is_kimi_thread_link(url) {
  const SUPPORTED_DOMAINS2 = [
    "kimi.com",
    "www.kimi.com"
  ];
  try {
    const u = new import_url.URL(url);
    if (!SUPPORTED_DOMAINS2.includes(u.hostname)) return false;
    const segments = (u.pathname || "").split("/").filter(Boolean).map((segment) => String(segment));
...
// src/utils/chatgpt_conversation_item.js
var parse_iso_time_to_ms = (iso_value) => {
  const raw_value = String(iso_value || "").trim();
  if (!raw_value) return 0;
  const ms = Date.parse(raw_value);
  return Number.isFinite(ms) ? ms : 0;
};
var get_chatgpt_conversation_sort_key_ms = (item) => {
  if (!item) return 0;
...
var DEFAULT_BASE_URL = "https://chatgpt.com";
function build_chatgpt_conversation_url(thread, opts = {}) {
...
function merge_chatgpt_conversation_items(existing = [], incoming = []) {
...
// src/utils/handle_chatgpt_threads_list_detection.js
var SC_NET_PREFIX = "[SC_NET]";
var try_parse_json = (raw_json) => {
  try {
...
function handle_chatgpt_threads_list_detection(codeblock_cls_instance) {
  if (!codeblock_cls_instance?.webview_el?.addEventListener) return;
  const update_detected_threads = (threads = []) => {
    const existing = Array.isArray(codeblock_cls_instance._detected_threads) ? codeblock_cls_instance._detected_threads : [];
  ...
  codeblock_cls_instance.webview_el.addEventListener("console-message", (e) => {
    ...
    if (request_url.includes("backend-api/conversations")) {
      ...
      console.log("Smart ChatGPT conversations response:", {
        current_url: codeblock_cls_instance.webview_el.getURL?.() || void 0,
        request_url,
...
var SmartChatCodeblock = class {
  ...
  _build_help_button(parent_el) {
    ...
    try {
      (0, import_obsidian45.setIcon)(btn, "help-circle");
    } catch (_) {
      btn.textContent = "?";
    }
    btn.onclick = () => {
  ...
  _is_mobile_app() {
    try {
      if (import_obsidian45.Platform?.isMobileApp) return true;
    } catch (_) {
    }
...
// src/utils/build_codex_diff_loader_execute_script.js
function codex_diff_loader() {
  (() => {
    const CFG = {
      poll_ms: 25,
      // fast polling for buttons
      settle_ms: 500,
      // stop after no new work for this long
      max_total_ms: 3e4,
      // hard stop
      scroll_each: true
      // helps trigger lazy-rendering in some UIs
    };
    const state = {
      clicked: /* @__PURE__ */ new WeakSet(),
      expanded: /* @__PURE__ */ new WeakSet(),
...
async function run_codex_diff_loader_when_ready(run_loader, opts = {}) {
  const cfg = {
    ...
    timeout_ms: Number.isFinite(Number(opts.timeout_ms)) ? Number(opts.timeout_ms) : 3e4
  };
  ...
  while (Date.now() - start_ms < cfg.timeout_ms) {
    ...
    await sleep2(cfg.poll_ms);
  }
  if (!same_route_as_expected()) return { ok: false, reason: "navigated_away" };
  run_loader();
  return { ok: false, reason: "timeout" };
}
var build_codex_diff_loader_execute_script = ({ expected_url }) => {
  const opts = {
...
// src/modals/add_thread_suggest_modal.js
var import_obsidian46 = require("obsidian");
var shorten_thread_id = (value) => {
  const s = String(value || "").trim();
  if (!s) return "";
  if (s.length <= 14) return s;
  return `${s.slice(0, 8)}...${s.slice(-4)}`;
};
var ChatgptThreadSuggestModal = class extends import_obsidian46.FuzzySuggestModal {
  /**
...
var CODEX_TASK_PATH_REGEX = /^\/codex\/tasks\/[a-z0-9-_]+\/?$/i;
var CODEX_HOSTNAMES = /* @__PURE__ */ new Set([
  "chatgpt.com",
  "chat.openai.com"
]);
var ADD_THREAD_BUTTON_LABEL = "Add thread";
var resolve_codex_diff_button_parent = ({ footer_parent_el, status_parent_el }) => {
  return footer_parent_el || status_parent_el || null;
};
var is_codex_task_url = (url) => {
  try {
    const u = new URL(url);
...
// src/views/smart_kimi_codeblock.js
var SmartKimiCodeblock = class extends SmartChatCodeblock {
  /**
   * @param {Object} options
   * @param {import('obsidian').Plugin} options.plugin - The parent plugin instance.
   * @param {import('obsidian').TFile} options.file - The file containing the codeblock.
...
var SmartChatGPTView = class extends import_obsidian47.ItemView {
...
async function wait_for_env_to_load(scope, opts = {}) {
  ...
  if (!wait_for_states.includes(scope.env?.state)) {
    ...
    while (scope.env.state === "init" && import_obsidian48.Platform.isMobile && !clicked_load_env) {
      if (container) {
        container.empty();
...
// node_modules/obsidian-smart-env/src/views/smart_plugin_settings_tab.js
var SmartPluginSettingsTab = class extends import_obsidian49.PluginSettingTab {
...
var SmartChatgptSettingTab = class extends SmartPluginSettingsTab {
  ...
  async render_plugin_settings(container) {
    if (!container) return;
    container.empty?.();
    new import_obsidian50.Setting(container).setName("Height (px)").setDesc("Iframe height for embedded webviews.").addText((txt) => {
    ...
    const zoom_setting = new import_obsidian50.Setting(container).setName("Zoom").setDesc("Zoom factor for all webviews.");
    ...
    new import_obsidian50.Setting(container).setName("Open WebUI URL").setDesc("Base URL for your Open WebUI instance (used by the smart-openwebui codeblock). Example: http://localhost:3000").addText((txt) => {
